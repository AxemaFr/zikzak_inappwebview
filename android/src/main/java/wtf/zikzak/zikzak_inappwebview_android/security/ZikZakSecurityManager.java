...
public class ZikZakSecurityManager {
    ...
    public static class SecurityPolicy {
        ...
        public SecurityPolicy() {
            this.securityHeaders = "default-src 'self'; script-src 'self' 'unsafe-inline'; object-src 'none';";
        }
    }
     ...
     */
    public static synchronized ZikZakSecurityManager getInstance(@NonNull Context context) {
        if (instance == null) {
    ...
    public void setSecurityEnhancementsEnabled(boolean enabled) {
        ...
        Log.d(TAG, "Security enhancements " + (enabled ? "enabled" : "disabled"));
    }
    ...
    public void applySecurityToWebView(final InAppWebView webView) {
        ...
        securityExecutor.execute(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    @RequiresApi(35)
    @SuppressLint("NewApi")
    private void applyAndroid15SecurityEnhancements(final InAppWebView webView) {
        // Apply newer security features only available in Android 15
        webView.post(new Runnable() {
            @Override
            public void run() {
                SecurityPolicy policy = getOrCreateSecurityPolicy(webView.getUrl());
                ...
                try {
                    ...
                    if (policy.enforceStrictCsp && WebViewFeature.isFeatureSupported(WebViewFeature.FORCE_DARK_STRATEGY)) {
                    ...
                    if (WebViewFeature.isFeatureSupported(WebViewFeature.REQUESTED_WITH_HEADER_ALLOW_LIST)) {
                        ...
                        allowList.add("https://*." + extractDomainFromUrl(webView.getUrl()));
                    ...
                    Log.d(TAG, "Applied Android 15+ security enhancements to WebView at " + webView.getUrl());
                } catch (Exception e) {
                    Log.e(TAG, "Error applying Android 15 security enhancements", e);
                }
            }
        });
    }
        ...
        webView.post(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.d(TAG, "Applied modern security enhancements to WebView at " + webView.getUrl());
                } catch (Exception e) {
                    Log.e(TAG, "Error applying modern security enhancements", e);
                }
            }
        });
    }
    ...
    private void applyLegacySecurityEnhancements(final InAppWebView webView) {
        webView.post(new Runnable() {
            ...
            public void run() {
                ...
                policy.enforceStrictCsp = policy.enforceStrictCsp && securityLevel >= SecurityLevel.MAXIMUM;
                policy.reduceFingerprinting = policy.reduceFingerprinting && securityLevel >= SecurityLevel.MAXIMUM;
                ...
                try {
                    ...
                    Log.d(TAG, "Applied legacy security enhancements to WebView at " + webView.getUrl());
                } catch (Exception e) {
                    Log.e(TAG, "Error applying legacy security enhancements", e);
                }
            }
        });
    }
    ...
    private String generateTrackerBlockingScript() {
        ...
        script.append("  const originalXHR = window.XMLHttpRequest.prototype.open;");
        
        // Override fetch
        script.append("  window.fetch = function(url, options) {");
        script.append("    if (shouldBlockRequest(url)) {");
        script.append("      console.log('[ZikZak Security] Blocked fetch request to ' + url);");
        script.append("      return Promise.reject(new Error('Request blocked by ZikZak Security'));");
        script.append("    }");
        script.append("    return originalFetch.apply(this, arguments);");
        script.append("  };");
        
        // Override XMLHttpRequest
        script.append("  window.XMLHttpRequest.prototype.open = function(method, url) {");
        script.append("    if (shouldBlockRequest(url)) {");
        script.append("      console.log('[ZikZak Security] Blocked XHR request to ' + url);");
        script.append("      throw new Error('Request blocked by ZikZak Security');");
        script.append("    }");
        ...
        script.append("    return blockedDomains.some(domain => urlString.includes(domain));");
        script.append("  }");
    ...
    private void updatePolicyForSecurityLevel(SecurityPolicy policy) {
        switch (securityLevel) {
            case SecurityLevel.MAXIMUM:
                ...
                policy.securityHeaders = "default-src 'self'; script-src 'self'; object-src 'none'; " +
            ...
            case SecurityLevel.ENHANCED:
                ...
                policy.securityHeaders = "default-src 'self' https:; script-src 'self' 'unsafe-inline' https:; " +
            ...
            default:
                ...
                policy.reduceFingerprinting = false;
                policy.securityHeaders = "default-src * 'unsafe-inline' 'unsafe-eval'; " +
